...
class MdrvTraceScore:
    ...
    max_router_score: float
    max_router_score_step: int | None
    max_margin_drawdown: float
    max_margin_drawdown_step: int | None

    @property
    def per_step(self) -> list[dict[str, Any]]:
        return [asdict(record) for record in self.records]


class MdrvTraceScorer:
    """Runs an SLM trace and scores every completed MDRV boundary."""

    def __init__(self, cfg: ExperimentConfig, slm: LocalQwen3Runner) -> None:
        self.cfg = cfg
        self.slm = slm
        self.delimiter = "\n\n"
    ...
    def _run_slm_trace(
        ...
        example: Example,
        ledger: CostLedger,
    ) -> tuple[list[int], list[int], str, str, list[StepBoundary], str, Any]:
    ...
    def score(
        ...
        max_router_score = 0.0
        max_router_score_step: int | None = None
        max_margin_drawdown = 0.0
        max_margin_drawdown_step: int | None = None

        attention_enabled = self.cfg.protocol.attention_route_mode == "last_layer_single_row"
        for boundary in boundaries:
            ...
            risk = (
                0.0
                if margin_update.margin_drawdown == 0.0
                else margin_update.margin_drawdown
            )
            ...
            takeover = (
                tau is not None
                and risk >= float(tau)
                and margin_update.margin_drawdown > 0.0
            )
            records.append(
                self._step_record(
                    boundary=boundary,
            ...
            if risk > max_router_score:
                max_router_score = float(risk)
                max_router_score_step = boundary.step_index
            if margin_update.margin_drawdown > max_margin_drawdown:
                max_margin_drawdown = float(margin_update.margin_drawdown)
                max_margin_drawdown_step = boundary.step_index
            if takeover:
                if not triggered:
                    triggered = True
        ...
        return MdrvTraceScore(
            ...
            max_router_score=max_router_score,
            max_router_score_step=max_router_score_step,
            max_margin_drawdown=max_margin_drawdown,
            max_margin_drawdown_step=max_margin_drawdown_step,
        )
